namespace SecondHandGear.Web.Net;
...
using System.Text;
using SecondHandGear.Web.Common;
using SecondHandGear.Web.Exception;
using SecondHandGear.Web.Response.Common;
...
public static class RestClient
{
    ...
    public static T Exchange<T>(string url, HttpMethod httpMethod,Dictionary<string,string>  headers, object request)  where T : class
    {
        ...
        catch(SecondHandGearWebException ex)
        {
            throw ex;
        }
        catch (Exception ex)
        {
            throw new SecondHandGearWebException(ex);
        }
    }

    private static T HandleResponse<T>(HttpResponseMessage httpResponseMessage) where T : class
    {
        var apiResponse = JsonSerializer.Deserialize<T>(httpResponseMessage.Content.ReadAsStringAsync().Result, SecondHandGearWebJsonSerializerSettings.Settings);
        ...
        //     throw new SecondHandGearWebException(errorResponse.ErrorCode!, errorResponse.ErrorDescription!, errorResponse.ErrorGroup!);
        // }
    ...
    public static StringContent PrepareContent(object request)
    {
        ...
        var json = JsonSerializer.Serialize(request,SecondHandGearWebJsonSerializerSettings.Settings);
